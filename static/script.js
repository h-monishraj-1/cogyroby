...
let worldState = {
    agent: {
        ...
        has_key: false,
        perception_radius: 5,
        exploration_target: {x: 39, y: 0},
        moving_direction: 'right',
        vertical_sweep_direction: 'down'
    },
    world_objects: []
};
...
function drawPerceptionRadius(agent) {
    const centerX = agent.x * CELL_SIZE + CELL_SIZE / 2;
    const centerY = agent.y * CELL_SIZE + CELL_SIZE / 2;
    const radius = agent.perception_radius * CELL_SIZE;
    
    // Draw perception radius as a semi-transparent circle
    ctx.save();
    ctx.fillStyle = 'rgba(173, 216, 230, 0.3)';
    ctx.beginPath();
    
    // Draw a diamond shape for Manhattan distance visualization
...
function drawObjects(objects) {
    // Now iterate through ALL objects, not just visible ones
    objects.forEach(obj => {
        // Calculate Manhattan distance from agent to object
        const distance = Math.abs(obj.x - worldState.agent.x) + Math.abs(obj.y - worldState.agent.y);
        ...
        // Check if object is within perception radius and adjust drawing accordingly
        if (distance > worldState.agent.perception_radius) {
            // Object is outside perception radius - draw it in "fog of war" style
            ctx.globalAlpha = 0.3; // Make it semi-transparent
        }
        // If within perception radius, globalAlpha remains at default 1.0
...
function draw() {
    ...
    // Draw everything
    drawGrid();
    drawPerceptionRadius(worldState.agent);
...
canvas.addEventListener('mousemove', (event) => {
    if (placingObjectType) {
        ...
        if (x >= 0 && x < GRID_WIDTH && y >= 0 && y < GRID_HEIGHT) {
            ...
            // Show if position is within perception radius
            const distance = Math.abs(x - worldState.agent.x) + Math.abs(y - worldState.agent.y);
            if (distance > worldState.agent.perception_radius) {
